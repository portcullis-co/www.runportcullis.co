...
import { ExpandableVoice } from "./ui/expandable-voice";
...
const RTVIWidget = ({
  ...
  useEffect(() => {
    return () => {
      if (rtviClient) {
        console.log("Cleaning up RTVI client...");
  ...
  const handleInitialClick = async () => {
    try {
      ...
      // Initialize devices before registering helpers
      try {
        ...
        if (defaultMic) {
          await client.updateMic(defaultMic.deviceId);
          console.log("Default microphone set:", defaultMic.label);
        }
      } catch (error) {
      ...
      // Add bot-specific event logging
      client.on(RTVIEvent.BotConnected, () => {
        console.log("Bot joined the session");
        // Enable microphone when bot connects
        client.enableMic(true);
        // Send a test message to verify bot interaction
        client.sendMessage({
          id: "test-message",
          type: "text",
      ...
      client.on(RTVIEvent.BotDisconnected, () => {
        console.log("Bot left the session");
